...
void apply_scale(int m, float sc);
void apply_color();
...
int main(int argc, char** argv)
{
	...
	while (1)
	{
		...
		if (d.size() == 3)
		{
			...
			int maxi = find_max();
			int mini = find_min();
			float diff = float(maxi - mini);
			float scale = float(255.0 / diff);
			apply_scale(mini, scale);
			apply_color();

			Mat Rgb = Mat(80, 60, CV_8UC1, &imgr);
			Mat rGb = Mat(80, 60, CV_8UC1, &imgg);
			Mat rgB = Mat(80, 60, CV_8UC1, &imgb);
...
void apply_color()
{
	const int *color_map = colormap_rainbow;
	for (int i = 0; i < 60; i++)
	{
		for (int j = 0; j < 80; j++)
		{
			imgr[i][j] = color_map[3 * img[i][j] + 0];
			imgg[i][j] = color_map[3 * img[i][j] + 1];
			imgb[i][j] = color_map[3 * img[i][j] + 2];
		}
	}
}